...
for %%f in (*.cpp) do (
    clang++ -cc1 -triple x86_64-scei-ps4-elf -I"%OO_PS4_TOOLCHAIN%\include" -I"%OO_PS4_TOOLCHAIN%\\include\\c++\\v1" -I"..\\External\\libjbc" -emit-obj -o %intdir%\%%~nf.o %%~nf.cpp
)
...
ld.lld -m elf_x86_64 -pie --script "%OO_PS4_TOOLCHAIN%\link.x" --eh-frame-hdr -o "%outputElf%" "-L%OO_PS4_TOOLCHAIN%\lib" %libraries% --verbose "%OO_PS4_TOOLCHAIN%\lib\crtlib.o" %obj_files% "..\\External\\libjbc\\jbc.o"
...
Rem Cleanup
copy "%outputPrx%" "%outputPath%\build\pkg\Daemons\ORBS30000\%targetname%.sprx"
del "%outputPrx%"
...
echo send "%outputPath%\build\pkg\Daemons\ORBS30000\%targetname%.sprx">> temp.txt
echo quit>> temp.txt
